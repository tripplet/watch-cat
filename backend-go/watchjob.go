...
type watchJob struct {
	...
	Uptime            time.Duration            `firestore:"uptime"`
	Enabled           bool                     `firestore:"enabled"`
	Status            string                   `firestore:"status"`
	Interval          int                      `firestore:"interval"`
	Secret            string                   `firestore:"secret"`
	LastIP            string                   `firestore:"last_ip"`
	LastIPv4          string                   `firestore:"last_ipv4"`
	LastIPv6          string                   `firestore:"last_ipv6"`
	TaskName          string                   `firestore:"task_name"`
	TimeoutActions    []*firestore.DocumentRef `firestore:"actions_timeout"`
	BackOnlineActions []*firestore.DocumentRef `firestore:"actions_back_online"`
	RebootActions     []*firestore.DocumentRef `firestore:"actions_reboot"`
}
